use clap::{crate_name, crate_version, crate_description, App, Arg, SubCommand};
...
fn create_basic_dir(name: &str) {
    ...
    std::process::Command::new("git")
        .args(&["init", name])
        .spawn().expect("Creating a git repository Failed.");
    let mut file = File::create(format!("{}/README.md", name)).expect("Could not create __main__.py");
    let boilerplate = format!("# {}", name);
    file.write_all(boilerplate.as_bytes()).expect("Failed to write to __main__.py");
}

fn create_bin_project(name: &str) {
    create_basic_dir(name);
    let mut file = File::create(format!("{}/src/__main__.py", name)).expect("Could not create __main__.py");
    let boilerplate =
...
fn create_lib_project(name: &str) {
    create_basic_dir(name);
    let _ = File::create(format!("{}/src/__init__.py", name)).expect("Could not create __main__.py");
}
...
fn cargs() -> Option<ProjectType> {
    ...
    if let Some(matches) = matches.subcommand_matches("new") {
        let project_name = matches.value_of("project_name").unwrap_or("default").to_string();
